...
/*#__PURE__*/
function (_Component) {
  _inherits(EntrySelect, _Component);

  function EntrySelect(props, contenx) {
    var _this;

    _classCallCheck(this, EntrySelect);

    _this = _possibleConstructorReturn(this, _getPrototypeOf(EntrySelect).call(this, props, contenx));
  ...
  _createClass(EntrySelect, [{
    ...
    value: function render() {
      ...
      if (value && labelInValue) {
        if ((0, _particulate.type)(value) === 'Array') {
          current = value.map(function (item) {
            return _objectSpread({}, item, {
              key: item.key || item[valueName],
              label: item.label || item[labelName]
            });
          });
        } else current = _objectSpread({}, value, {
          key: value.key || value[valueName],
          label: value.label || value[labelName]
        });
      }

      return _react.default.createElement(_antd.Select, _extends({}, this.props, {
        value: current
...
_defineProperty(EntrySelect, "defaultValue", undefined);

var _default = EntrySelect;
